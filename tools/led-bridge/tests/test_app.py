...
class WorkerDisplayTest(unittest.TestCase):
    ...
    def test_ready_order_becomes_last_call_when_threshold_is_reached(self):
        stop_event = threading.Event()
        panel = FakePanel()
        queue = ScriptedQueue(stop_event, [event("order-40", 40, "ready")])
        ...
        sent_names = [path.name for path in panel.sent]
        self.assertIn("last-call-order-40.png", sent_names)

    def test_closed_stand_shows_closed_screen(self):
        stop_event = threading.Event()
        stop_event.set()
        panel = FakePanel()
        queue = ScriptedQueue(stop_event, [])
        ...
        sent_names = [path.name for path in panel.sent]
        self.assertIn("stand-closed.png", sent_names)
